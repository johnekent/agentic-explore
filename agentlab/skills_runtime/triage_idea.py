...
@dataclass
class TriageIdea(Skill):
    spec = SkillSpec(name="triage_idea", required_capabilities=["cap.llm_generate"])

    def run(self, orch, **kwargs) -> Dict[str, Any]:
        system = "You triage ideas into short labels. Return JSON {\"priority\":\"low|medium|high\"}."
